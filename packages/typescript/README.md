...
idiomatic Zod counterpart to the Python/Pydantic package and is held to exact behavioral
parity with it: the same CLI inputs/outputs/flags, the same library surface, and the same
...
[Softschema Guide](../../docs/softschema-guide.md) / [Spec](../../docs/softschema-spec.md)
for the language-neutral concept and artifact format.

## Development

bun + bunup + biome. `bun run check` runs lint, types, and tests. The shared golden
corpus (`tests/golden/`) runs against this CLI via `SOFTSCHEMA_IMPL=ts`; a
cross-implementation conformance test asserts the Zod and Pydantic compilers produce an
identical canonical sidecar. See the parity development process in
