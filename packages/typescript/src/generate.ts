...
function resolveContractPath(contract: string, base: string): string {
  const path = isAbsolute(contract) ? contract : join(base, contract);
  return path;
}
...
function renderSection(section: GeneratedSection, contractRoot: string): string {
  const kind = section.attrs.kind ?? "";
  const contract = section.attrs.contract ?? "";
  if (!kind) throw new Error("softschema:generated marker is missing 'kind'");
  if (!contract) throw new Error("softschema:generated marker is missing 'contract'");
  ...
  const schemaPath = resolveContractPath(contract, contractRoot);
...
export function regenerate(
  path: string,
  options: { check?: boolean; contractRoot?: string } = {},
): RegenerateResult {
