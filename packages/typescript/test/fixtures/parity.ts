...
export const KitchenSink = z
  .strictObject({
    ...
    rank: z.enum(["low", "high"]).nullable().optional(),
    primary: Address,
    secondary: Address.nullable().optional(),
    history: z.array(Event).default([]),
    last_event: Event.nullable().optional(),
    scores: z.record(z.string(), z.int()).default({}),
