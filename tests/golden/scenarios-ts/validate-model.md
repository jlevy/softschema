...
# Test: pure YAML reaches the Zod semantic layer

The same language-neutral movie payload validates through the explicit `pure-yaml`
profile. The model spec is the only runtime-specific part of this command.

```console
$ softschema validate examples/movie_page/spirited-away.yaml --profile pure-yaml --model packages/typescript/test/fixtures/movie-model.mjs:MoviePage
{
  "contract": {
    "envelope_key": null,
    "id": "example.movies:MoviePage/v1",
